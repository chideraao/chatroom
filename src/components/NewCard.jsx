...
function NewCard() {
  ...
  const createNewChat = (e) => {
    ...
    let errorMessage = (userEmail) => {
      searchRef.current.innerHTML = `User '${userEmail}' does not yet exist. <div class='modal-btn flex'><button aria-label='Cancel' title='Cancel' id='cancel-btn'>Cancel</button>  <button aria-label='invite' title='Invite' id='mail-btn'>Invite</button> </div> `;
      document
